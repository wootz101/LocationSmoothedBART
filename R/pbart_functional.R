...
pbart_functional=function(
    num_predictors, num_intvls,
    ...
    dart_fp=FALSE, theta_fp=0, omega_fp=1,
    a_fp=0.5, b_fp=1, augment_fp=FALSE, rho_fp=NULL,

    dart_int=FALSE, theta_int=0, omega_int=1,
    a_int=0.5, b_int=1, augment_int=FALSE, rho_int=NULL,

    fbart_conditional = FALSE,
...
)
{
  ...
  # // updating RHO for predictor and intervals
  if(length(rho)==0) rho <- p
  if(length(rho_fp)==0) rho_fp <- num_predictors
  if(length(rho_int)==0) rho_int <- num_intvls
  ...
  ptm <- proc.time()
  #call
  res = .Call("cpbart_functional",
              num_predictors, #number of functional predictors
              num_intvls, #number of points or intervals of function
  ...
  if(fbart_conditional){
    l1 = list();
    for (i in 1:num_predictors) {
      #print(i)
      #print(dim(res$varprob_int))
      l1[[i]]=res$varprob_int[ , (num_intvls*(i-1)+1):(num_intvls*i)]
    }
    res$varprob_times <- l1
  } #else res$varprob_int <- res$varprob_int[1:num_intvls]
