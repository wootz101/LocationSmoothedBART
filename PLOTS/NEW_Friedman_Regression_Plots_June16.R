...
noise_variance <- signal_variance / 11
sqrt(noise_variance)

set.seed(314)
true_y = true_signal + rnorm(length(true_signal), sd = sqrt(noise_variance))
...
for (num_p in c(1:10) ) {
  ...
  if(num_p>1){
    for(i in 1:(num_predictors-5) ){
      set.seed(i)
      d_rand_points = matrix(rnorm(dim(d1_points)[1]*dim(d1_points)[2], sd=1),
  ...
  # Perform PCA because to many predictor variables
  ...
  nn_model <- nnet(y_train ~ ., data = train_pca,
                   size = 5, maxit = 100, decay = 0.1,
...
data2 <- data.frame(
  ...
  y6 = as.numeric(lasso_time_all),
  y7 = as.numeric(ridge_time_all),
  y8 = as.numeric(nn_time_all)
...
ggplot(data, aes(x = x)) +
  ...
  geom_line(aes(y = y8, color = "NNet"), size = 1.5) +
  geom_point(aes(y = y8, color = "NNet"), size = 3) +
...
names(data2) = c("x", "F. Reg.","lsBART", "BART", "DART", "RF", "LASSO", "Ridge", "NN" )
data2
colMeans(data2)
