...
############################################################################
flog_num_predictors = 5

tempbasis = create.bspline.basis(rangeval = c(0,1), nbasis=8)
grid <- seq(0,1,1/(length(df[1,1:n_point])-1))

templist      = vector("list", flog_num_predictors+1)
templist[[1]] = rep(1,  nrow(df))

for (i in 0:(flog_num_predictors-1) ) {
  tempSmooth= smooth.basis(grid, t(df[,(n_point*i+1):(n_point*i+n_point)]), tempbasis)
...
var_noise
# Calculate the variance of the fitted values (signal)
...
# Print SNR and SNR in dB
print(paste("SNR:", snr))
print(paste("SNR in dB:", snr_db))




# Fit a logistic regression model
model <- glm(true_y ~ points, family = binomial(link = "logit"))
...
# Prepare your data: Assuming 'points' is correctly formatted as a matrix
if (!is.matrix(points)) {
  points <- as.matrix(points)
}

# Convert 'true_y' to a factor if it isn't already
true_y <- factor(true_y)

# Cross-validate to find the optimal lambda
cv_model <- cv.glmnet(points, as.numeric(true_y) - 1, family = 'binomial', type.measure = "class")
cv_model$lambda.1se

# Fit a logistic regression model using glmnet with the lambda.1se from cross-validation
model <- glmnet(points, as.numeric(true_y) - 1, family = 'binomial', alpha = 1, lambda = cv_model$lambda.1se)
summary(model)
model$beta

# Predict probabilities using the lambda.1se
predicted_probs <- predict(model, newx = points, type = "response", s = cv_model$lambda.1se)
predicted_probs
plot(predicted_probs)

# Calculate logits with stability
logits <- log(predicted_probs / (1 - predicted_probs + 1e-5))  # Adding a small constant to avoid log(0)

# Calculate variance of the logits (signal strength)
signal_variance <- var(logits)
signal_variance
# Calculate theoretical noise variance (mean of variances of Bernoulli trials)
noise_variance <- mean(predicted_probs * (1 - predicted_probs))
noise_variance
# Calculate SNR
snr <- signal_variance / noise_variance
