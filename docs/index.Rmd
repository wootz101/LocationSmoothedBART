...
d1_points = t(d1$coefs) %*% t(eval.basis(seq(
  by = 0.01, from = 0, to = 1
), bb))

d2 = Data2fd(c2_s_data, basis = bb)
d2_points = t(d2$coefs) %*% t(eval.basis(seq(
  by = 0.01, from = 0, to = 1
), bb))

d3 = Data2fd(c3_s_data, basis = bb)
d3_points = t(d3$coefs) %*% t(eval.basis(seq(
  by = 0.01, from = 0, to = 1
), bb))

d4 = Data2fd(c4_s_data, basis = bb)
d4_points = t(d4$coefs) %*% t(eval.basis(seq(
  by = 0.01, from = 0, to = 1
), bb))

d5 = Data2fd(c5_s_data, basis = bb)
d5_points = t(d5$coefs) %*% t(eval.basis(seq(
  by = 0.01, from = 0, to = 1
), bb))
...
friedmanFunc <- function(x) {
  res = 10 * sin(pi * x[1] * x[2]) + 20 * (x[3] - 0.5) ^ 2 + 10 * x[4] + 5 *
    x[5]
  return(res)
}
...
par(mfrow = c(1, 1))
barplot(
  colMeans(m0$varprob_fp),
  main = "Functional Predictor Probabilities",
  space = 0,
  names.arg  = paste0("X_", 1:num_predictors)
)
...
abline(
  v = (ts1[[1]]) / dim(d1_points)[2],
  lwd = 1,
  col = "green",
  lty = 2
)
barplot(
  colMeans(m0$varprob_times[[1]]),
  main = "X_1 Location Probability",
  names.arg = as.character(1:num_intvls),
  space = 0
)
abline(
  v = (ts1[[1]]),
...
abline(
  v = (ts1[[2]]) / dim(d1_points)[2],
  lwd = 1,
  col = "green",
  lty = 2
)
barplot(
  colMeans(m0$varprob_times[[2]]),
  main = "X_2 Location Probability",
...
Must have TreeShap_Source file loaded to run TreeShap algorithm on BART model. The file is found in the /docs folder.
...
var_names2 = c("X_1", "X_2", "X_3", "X_4")
barplot(colMeans(unif_list2),
        main = paste("Shapely Value Average Obs: ", obs_id))
...
for (i in 1:num_predictors) {
  barplot(
    colMeans(unif_list2)[(num_intvls * (i - 1) + 1):(num_intvls * i)],
    names.arg  = as.character(round((1:num_intvls) / num_intvls, 2)),
    main = paste("Obs-", obs_id, var_names2[i]),
    ylim = c(min(colMeans(unif_list2)), max(colMeans(unif_list2)))
  )
  
  shape_spahely_total[i] = sum(colMeans(unif_list2)[(num_intvls * (i - 1) +
                                                       1):(num_intvls * i)])
}
par(mfrow = c(1, 1))
...
### Shapley plot for Functional Predictor importance
...
Here we see which predictor function had the largest influence on the prediction of observation 1. It turns out to be X_2. From this we infer that X_2 was the most important predictor function for the observation 1.


### Shapley plot for location importance of X_2

Finally, we examine the Shapely values of observation 1's X_2 predictor function to identify the specific regions within X_2 that were most influential in the classification.
...
plot_points_imp = colMeans(unif_list2)[(num_intvls * (i - 1) + 1):(num_intvls *
                                                                     i)]
...
df_shape_loc11 = tibble(
  percentage = c(0, pd$basis$params, 1),
...
ggplot(df_shape_loc11, aes(x = percentage)) +
  geom_bar(aes(y = shapely * multiplier,  fill = shapely > 0),
           stat = "identity",
           alpha = 0.7) +
  geom_line(
    aes(y = line),
