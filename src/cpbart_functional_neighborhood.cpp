...
RcppExport SEXP cpbart_functional_neighborhood(
    SEXP _funcP,
    SEXP _intvls,
    ...
    //Choose whether or not condtional or independt time variabls
    //SEXP _iconditional,
...
)
{
 ...
 // bool fbart_cond;
 // if(Rcpp::as<int>(_iconditional)==1) fbart_cond=true;
...
#define TEDRAW(a, b) tedraw[a][b]
  ...
  )
  {
    ...
    bm.setdata_fbart(p,n, funcP, intvls, ix,iz,numcut);
    ...
    for(size_t i=0;i<(nd+burn);i++) {
      ...
      //draw with either conditional or independent t

      //bm.draw_global_conditional(funcP, intvls, 1.0, gen, gen_fp, gen_int);

      bm.draw_global_neighborhood(funcP, intvls, 1.0, gen, gen_fp, gen_int, sigma_int);
      //bm.draw_global(funcP, intvls, 1.0, gen, gen_fp, gen_int);
      //if(i%printevery==0) printf("draw_global\n");


      //bm.draw_conditional_neighborhood(funcP, intvls, 1.0, gen, gen_fp, gen_int);
      ...
      /*
       ...
       if(fbart_cond){
       bm.draw_global_conditional(funcP, intvls, 1.0, gen, gen_fp, gen_int);
       } else bm.draw_global(funcP, intvls, 1.0, gen, gen_fp, gen_int);
       */
      ...
      if(i>=burn) {
        ...
        if(keeptreedraw) {
          ...
          /*
           ...
           ivarcnt_int = bm.getnv_intvls();
           ivarprb_int = bm.getpv_intvls();
          ...
          /*
           ...
           varcnt_int.push_back(bm.getnv_intvls());
           varprb_int.push_back(bm.getpv_intvls());
